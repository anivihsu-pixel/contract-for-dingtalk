...
- [x] 合同列表/详情「我方身份」误标修复（2026-09-16，v2.51.17）：详情「合同方向」行「我方为」与列表方向标签原按收付款方向（direction）硬推身份（sales→甲方、purchase→乙方），导致「我方=乙方+收款」的技术服务合同错误显示「我方为：甲方」；改为与 v2.51.13 甲乙方标签同口径，按 v2.46.0「对方侧必关联档案」约束反推我方身份（单侧关联→我方在另一侧，双侧同有关/同无关安全降级不标注），列表 dirBadge 仅表达收付方向不再写身份。验收：php -l/node --check 通过；服务端详情 id23（我方乙方）渲染「我方为：乙方」、id21（我方甲方）「我方为：甲方」；列表 contract.js 经 asset_url(mtime) 版本化（v=1789536477）自动加载新 code，无「我方为甲方」残留。DB 门禁通过。无 DB 变更
