-- +----------------------------------------------------------------------
-- | 迁移脚本 v2.40.5：补齐三张表缺失索引（P2-5 架构审查）
-- | 关联任务：
-- |   - customer_claim_record(user_id, created_at)：支撑认领每日限额查询 CustomerLogic::claim
...
CREATE INDEX IF NOT EXISTS idx_claim_user_created   ON customer_claim_record(user_id, created_at);
CREATE INDEX IF NOT EXISTS idx_activity_customer    ON customer_activity(customer_id);

-- ---------- MySQL 执行（去掉上方 SQLite 语句，改用下方）----------
-- 注：customer_activity 的 idx_activity_customer 自建表即存在，MySQL 无需再建，仅补前两个
-- ALTER TABLE `user` ADD INDEX idx_user_dept (dept_id);
-- ALTER TABLE `customer_claim_record` ADD INDEX idx_claim_user_created (user_id, created_at);

-- 检查（SQLite 下执行）：
-- SELECT name FROM sqlite_master WHERE type='index' AND name IN ('idx_user_dept','idx_claim_user_created','idx_activity_customer');
