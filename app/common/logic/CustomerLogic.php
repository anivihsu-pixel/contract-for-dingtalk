...
class CustomerLogic
{
    ...
    /**
     * v2.38.3：根据逾期回款重算客户信用评级
     * 规则：
     *  - 信用评分 = max(0, 100 - 逾期笔数*20)
     *  - 高风险 = 逾期 >=3 笔 或 存在 90 天以上逾期
     * @param int $customerId
     * @return array ['score'=>int,'high_risk'=>int,'overdue_count'=>int,'overdue_90'=>int]
     */
    public static function recalcCreditScore(int $customerId): array
    {
        $now = time();
        $rows = Db::name('payment_record')->alias('p')
            ->join('contract c', 'p.contract_id = c.id')
            ->where('c.party_b_customer_id', $customerId)
            ->where('c.is_deleted', 0) // M8 修复：软删合同的逾期不计入评级
            ->where('p.status', 'OVERDUE')
            ->field('p.planned_date')
            ->select()->toArray();
        $overdueCount = count($rows);
        $overdue90 = 0;
        foreach ($rows as $r) {
            $pd = strtotime((string)($r['planned_date'] ?? ''));
            if ($pd !== false && ($now - $pd) >= 90 * 86400) {
                $overdue90++;
            }
        }
        $score = max(0, 100 - $overdueCount * 20);
        $highRisk = ($overdueCount >= 3 || $overdue90 > 0) ? 1 : 0;

        // M8 修复：人工锁定（credit_manual=1，用户改过评分）时自动重算跳过评分，仅刷新客观风险标记
        $cust = Db::name('customer')->where('id', $customerId)->find();
        $update = [
            'credit_score' => $score,
            'high_risk'    => $highRisk,
        ];
        if (!empty($cust['credit_manual'])) {
            unset($update['credit_score']);
        }
        Db::name('customer')->where('id', $customerId)->update($update);
        return [
            'score'         => $score,
            'high_risk'     => $highRisk,
    ...
    /** v2.38.3：批量重算全部非本公司客户的信用评级（供命令调用），返回统计 */
    public static function recalcAllCreditScores(): array
    {
        $ids = Db::name('customer')->where('is_self', 0)->where('is_deleted', 0)
            ->column('id');
        $stats = ['total' => count($ids), 'high_risk' => 0];
        foreach ($ids as $id) {
            $r = self::recalcCreditScore((int)$id);
            if ($r['high_risk']) {
                $stats['high_risk']++;
            }
        }
