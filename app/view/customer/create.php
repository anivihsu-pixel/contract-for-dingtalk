<?php $title=$customer?'编辑客户':'新增客户'; $menu_active='customer_create'; include __DIR__.'/../layout/header.php'; ?>
...
<div class="col-md-3 col-6"><label class="form-label" for="fSource">来源 <span class="text-danger">*</span></label><select name="source" id="fSource" class="form-select"><option value="" <?=empty($customer['source']??'')?'selected':''?>>请选择客户来源</option><option value="MANUAL" <?=($customer['source']??'')=='MANUAL'?'selected':''?>>手动录入</option><option value="WEBSITE" <?=($customer['source']??'')=='WEBSITE'?'selected':''?>>官网</option><option value="EXHIBITION" <?=($customer['source']??'')=='EXHIBITION'?'selected':''?>>展会</option><option value="REFERRAL" <?=($customer['source']??'')=='REFERRAL'?'selected':''?>>转介绍</option><option value="PHONE" <?=($customer['source']??'')=='PHONE'?'selected':''?>>电话</option><option value="AD" <?=($customer['source']??'')=='AD'?'selected':''?>>广告</option><option value="OTHER" <?=($customer['source']??'')=='OTHER'?'selected':''?>>其他</option></select></div>
...
<!-- 客户生命周期（补全 M10 漏斗编辑入口）——客户/成交 -->
<div class="col-md-3 col-6"><label class="form-label" for="fLifecycle">生命周期</label><select name="lifecycle_status" id="fLifecycle" class="form-select"><?php foreach(($lifecycle_dict??['POTENTIAL'=>'客户','ACTIVE'=>'成交']) as $code=>$label): ?><option value="<?=htmlspecialchars($code)?>" <?=($customer['lifecycle_status']??'POTENTIAL')===$code?'selected':''?>><?=htmlspecialchars($label)?></option><?php endforeach; ?></select></div>
<!-- v2.40.0 P1-7：客户行业（政府单位/房地产/餐饮旅游/其他） -->
<div class="col-md-3 col-6"><label class="form-label" for="fIndustry">行业 <span class="text-danger">*</span></label><select name="industry" id="fIndustry" class="form-select"><option value="">请选择客户行业</option><?php foreach(($industry_dict??[]) as $code=>$label): ?><option value="<?=htmlspecialchars($code)?>" <?=($customer['industry']??'')===$code?'selected':''?>><?=htmlspecialchars($label)?></option><?php endforeach; ?></select></div>
<div class="col-12"><label class="form-label" for="fRemark">备注</label><input type="text" name="remark" id="fRemark" class="form-control" maxlength="255" placeholder="简短备注" value="<?=htmlspecialchars($customer['remark']??'')?>"></div>
...
<script>document.getElementById('customerForm').addEventListener('submit',function(e){e.preventDefault();var fd=new FormData(this);if(!fd.get('id')){if(!fd.get('source')){showToast('请选择客户来源','error');return;}if(!fd.get('industry')){showToast('请选择客户行业','error');return;}}fetch('/ajax/customer/save',{method:'POST',body:new URLSearchParams(fd),headers:{'X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(res=>{if(res.code===0){window.__formDirty=false;location.href='/customer/'+res.data.id;}else showToast(res.msg,'error');});});</script>
