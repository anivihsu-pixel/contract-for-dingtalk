...
class Template
{
    ...
    public function fetch(string $template, array $vars = []): void
    {
        ...
        if ($template) {
            ...
            if (!$this->checkCache($cacheFile)) {
                ...
                // 修复：将模板源中的 __DIR__/__FILE__ 魔术常量替换为源文件实际路径字符串。
                // 否则编译缓存写入 runtime/temp 后，运行时 include 该缓存文件时 __DIR__ 指向 temp 目录，
                // 导致视图内 `include __DIR__.'/../layout/...'` 布局包含全部失败（所有用布局的页面 500）。
                // 旧版 think-template 曾有此替换、新版缺失；此处恢复以保证 __DIR__ 始终指向视图源目录。
                $content = preg_replace('/\b__DIR__\b/', var_export(dirname($template), true), $content);
                $content = preg_replace('/\b__FILE__\b/', var_export($template, true), $content);
    ...
    private function compiler(string &$content, string $cacheFile): void
    {
        ...
        // 合并相邻PHP块时保留分号+换行：换行可阻断单行注释(//)吞掉后续语句
        $content = preg_replace('/\?>\s*<\?php\s(?!echo\b|\bend)/s', ";\n", $content);
